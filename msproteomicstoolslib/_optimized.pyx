...
def c_mcmcrun2(int nrit, dict selection_vector, list tree_path, 
               bytes tree_start, dict pg_per_run, mpep,
               tr_data, int n_runs, double transfer_width, double f=1.0, 
               bool verbose=False, bool biasSelection=False, bool usecpp=True):

        cdef double delta_score, score, best_score, prev_score
        cdef int BIAS_PEAKGROUPS
        cdef bytes select_run
        cdef int select_pg
        cdef int i
        cdef int pg_, pg
        ...
        for i in range(nrit):
            ...
            select_run = pg_per_run.keys()[ getRandomInt(0, n_runs-1)  ]
            print "random int between %s and %s: %s" % (0, n_runs-1, getRandomInt(0, n_runs-1)) 
            # print "seelect run", select_run
